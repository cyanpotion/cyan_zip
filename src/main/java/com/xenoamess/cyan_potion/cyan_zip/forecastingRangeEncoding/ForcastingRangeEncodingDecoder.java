...
public class ForcastingRangeEncodingDecoder {
    ...
    protected BitInput bitInput;
    protected Transition transition;

    protected long rangeL;
    protected long rangeR;
    protected long rangeN;
...
//        }
    ...
    public void preTransition() {
        if (this.byteBuffer.size() > ForcastingRangeEncodingEncoder.ByteBufferMax) {
            ...
            this.transition.sub(0, byte3, byte2,
                    (ForcastingRangeEncodingEncoder.MultiTime << (ForcastingRangeEncodingEncoder.TRANSITION_NUM - 1 - 0)));
            this.transition.sub(1, byte3, byte1,
                    (ForcastingRangeEncodingEncoder.MultiTime << (ForcastingRangeEncodingEncoder.TRANSITION_NUM - 1 - 1)));
            this.transition.sub(2, byte3, byte0,
                    (ForcastingRangeEncodingEncoder.MultiTime << (ForcastingRangeEncodingEncoder.TRANSITION_NUM - 1 - 2)));
        }
    }
    ...
    public int calculateByte() throws IOException {
        ...
        while (Long.compareUnsigned(Long.divideUnsigned(nowRange, transitionSize),
                ForcastingRangeEncodingEncoder.RangeTime) < 0) {
            rangeL <<= 1;
            rangeR <<= 1;
            rangeN <<= 1;
//            rangeR |= 1L;
    ...
    public void updateTransition(int currentByte) {
        ...
        if (this.byteBuffer.size() >= 3) {
            ...
            this.transition.add(2, byte3, currentByte,
                    (ForcastingRangeEncodingEncoder.MultiTime << (ForcastingRangeEncodingEncoder.TRANSITION_NUM - 1 - 2)));
...
//            this.transition_b[currentByte] += MultiTime << (TRANSITION_NUM - 1 - 2);
        ...
        if (this.byteBuffer.size() >= 2) {
            ...
            this.transition.add(1, byte2, currentByte,
                    (ForcastingRangeEncodingEncoder.MultiTime << (ForcastingRangeEncodingEncoder.TRANSITION_NUM - 1 - 1)));
...
//            this.transition_b[currentByte] += MultiTime << (TRANSITION_NUM - 1 - 1);
        ...
        if (this.byteBuffer.size() >= 1) {
            ...
            this.transition.add(0, byte1, currentByte,
                    (ForcastingRangeEncodingEncoder.MultiTime << (ForcastingRangeEncodingEncoder.TRANSITION_NUM - 1 - 0)));
